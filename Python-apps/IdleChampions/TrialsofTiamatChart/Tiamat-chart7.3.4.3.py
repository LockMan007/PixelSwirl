...
# --- CONFIGURATION / CONSTANTS, my own personal variable, misspelled intentionally ---
VERSIONNNUUMBER = "7.3.4.3"
...
def get_current_plot_filepath():
    """Derives save path based on start_time of run, avoiding Sunday midnight mid-run file switches."""
    ensure_plot_dir()
    if start_time:
        week_str = start_time.strftime("%Y_W%U")
    else:
...
def get_historical_finish_times():
    """Helper function to parse all completion hours from stored run files."""
    ensure_plot_dir()
        ...
        try:
            with open(filepath, "r") as f:
                ...
                if data and isinstance(data, list) and len(data) > 0:
                    last_pt = data[-1]
                    rx, ry = last_pt[0], last_pt[1]
                    last_dps = float(last_pt[2]) if len(last_pt) > 2 else 0.0
...
def load_all_historical_runs():
    """Loads historical run files and extends trajectories to 0% HP."""
    global CACHED_HISTORICAL_RUNS
            ...
            with open(filepath, "r") as f:
                ...
                if data and isinstance(data, list) and len(data) > 0:
                    ...
                    if ry <= 1.0:
                        ...
                        finish_x = rx + h_to_kill
                        if finish_x <= 168.0:
                            successful_finishes.append(finish_x)
        except Exception:
            continue
    ...
    for run in all_runs:
        ...
        if len(run) > 0 and ry[-1] > 0:
            last_x = rx[-1]
            last_y = ry[-1]
            last_dps = float(run[-1][2]) if len(run[-1]) > 2 else 0

            if last_dps > 0:
                tier_val = int(strip_non_numeric(tier_entry.get())) if 'tier_entry' in globals() else 10
                total_hp = TIER_DATA.get(tier_val, TIER_DATA[10])["health"]
                rem_hp = (last_y / 100.0) * total_hp
                h_to_kill = (rem_hp / last_dps) / 3600.0
                end_x = last_x + h_to_kill
                rx.append(end_x)
                ry.append(0.0)
    ...
    if successful_finishes:
        earliest_finish = min(successful_finishes)
        min_y[gx >= earliest_finish] = 0.0
...
def update_performance_gauge(current_x, current_y):
    ...
    y_best = float(np.interp(current_x, gx, min_y))
    y_worst = float(np.interp(current_x, gx, max_y))
...
def draw_vertical_progress_bar(canvas, ratio, fill_color="#007ACC", bg_color="white"):
    """Draws a vertical progress bar filling from bottom up representing remaining proportion."""
    canvas.delete("all")
    ...
    canvas.create_rectangle(0, 0, w - 1, h - 1, fill=bg_color, outline="black")

    clamped_ratio = max(0.0, min(1.0, ratio))
    if clamped_ratio > 0:
        fill_h = int(clamped_ratio * (h - 2))
        top_y = (h - 1) - fill_h
        canvas.create_rectangle(1, top_y, w - 1, h - 1, fill=fill_color, outline="")
...
def run_update():
    global tracking_id, history, LAST_ALL_DAYS_RATIO, LAST_DAY_RATIO
    try:
        tier_val = int(strip_non_numeric(tier_entry.get()))
        ...
        # 1. Update All-Days Bar (Out of 7 days / 168 hours total)
        LAST_ALL_DAYS_RATIO = max(0.0, min(1.0, current_time_left / 604800.0))
        draw_vertical_progress_bar(all_days_canvas, LAST_ALL_DAYS_RATIO)

        # 2. Update Day Cycle Bar & Day Left Label
        delta = timedelta(seconds=int(max(0, current_time_left)))
        day_seconds = delta.seconds
        LAST_DAY_RATIO = max(0.0, min(1.0, day_seconds / 86400.0))
        draw_vertical_progress_bar(day_progress_canvas, LAST_DAY_RATIO)

        day_h = day_seconds // 3600
        day_m = (day_seconds // 60) % 60
        day_tracking_label.config(text=f"Day left:\n{day_h}h {day_m}m")
...
def schedule_blue_tracker_update(*args):
    global blue_tracker_timer_id
    if blue_tracker_timer_id is not None:
        root.after_cancel(blue_tracker_timer_id)
    blue_tracker_timer_id = root.after(300, _trigger_blue_tracker_redraw)


def _trigger_blue_tracker_redraw():
    global blue_tracker_timer_id
    blue_tracker_timer_id = None
    update_chart()
...
def reset_run():
    ...
    global LAST_ALL_DAYS_RATIO, LAST_DAY_RATIO
    ...
    if messagebox.askyesno(
        ...
        day_tracking_label.config(text="Day left:\n--")

        LAST_ALL_DAYS_RATIO = 0.0
        LAST_DAY_RATIO = 0.0
        draw_vertical_progress_bar(all_days_canvas, 0.0)
        draw_vertical_progress_bar(day_progress_canvas, 0.0)
...
def block_trace_handlers(should_block):
    ...
    if should_block:
        ...
        dps_trace_id = dps_var.trace_add("write", schedule_blue_tracker_update)
        hp_trace_id = hp_var.trace_add("write", schedule_blue_tracker_update)
        hp_sub_trace_id = hp_sub_var.trace_add("write", schedule_blue_tracker_update)
        d_trace_id = d_var.trace_add("write", schedule_blue_tracker_update)
        h_trace_id = h_var.trace_add("write", schedule_blue_tracker_update)
        m_trace_id = m_var.trace_add("write", schedule_blue_tracker_update)


# --- GUI SETUP ---
root = tk.Tk()
root.title(f"ToMT Tier Progress Calculator (v{VERSIONNNUUMBER})")
...
# Container for tracking labels and vertical blue progress bars
tracking_container = tk.Frame(root)
tracking_container.grid(row=2, column=0, columnspan=4, pady=5)

# 1. All-Days Blue Progress Bar Canvas (Left)
all_days_canvas = tk.Canvas(
    tracking_container, width=14, height=40, highlightthickness=0
)
all_days_canvas.pack(side=tk.LEFT, padx=(5, 2))

# 2. Main Live Tracking Yellow Box
...
live_tracking_label.pack(side=tk.LEFT, padx=(2, 5))

# 3. Green/Blinking Day Status Indicator
...
# 4. Day Left Yellow Box
day_tracking_label = tk.Label(
    tracking_container,
    text="Day left:\n--",
    font=("Arial", 10),
    bg="lightyellow",
...
day_tracking_label.pack(side=tk.LEFT, padx=(5, 2))

# 5. Current Day Cycle Blue Progress Bar Canvas (Right)
day_progress_canvas = tk.Canvas(
    tracking_container, width=14, height=40, highlightthickness=0
)
day_progress_canvas.pack(side=tk.LEFT, padx=(2, 5))
