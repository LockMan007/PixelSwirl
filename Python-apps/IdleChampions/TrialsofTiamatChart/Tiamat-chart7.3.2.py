...
def get_historical_finish_bounds():
    """
    Dynamically extracts the fastest and slowest completion times (in hours)
    from all historical dataset files where a run successfully cleared Tiamat.
    """
    ensure_plot_dir()
        ...
        try:
            with open(filepath, "r") as f:
                ...
                if data and isinstance(data, list) and len(data) > 0:
                    ...
                    # If run ended at 0% HP, record the end time directly
                    if ry <= 1.0:
                        successful_finishes.append(rx)
                    # If run cut off before 0%, project when it would have cleared
                    elif last_dps > 0:
    ...
    if successful_finishes:
        return min(successful_finishes), max(successful_finishes)
    return 0.0, 168.0
...
def load_all_historical_runs():
    ...
    for run in all_runs:
        ...
        # Extend historical trajectory down to 0% HP if run cut off early
    ...
    # Automatically zero out min_y past the earliest finish time to fill the bottom area
    if successful_finishes:
...
def update_performance_gauge(current_x, current_y):
    ...
    # rel_pos near 1.0 = High HP = WORST (Top of canvas / Red)
    # rel_pos near 0.0 = Low HP = BEST (Bottom of canvas / Green)
    rel_pos = (current_y - hist_min) / (hist_max - hist_min)

    h = 110
    clamped_pos = max(-0.15, min(1.15, rel_pos))
    
    # 0 pixels is the very top (red), h pixels is the bottom (green)
    ...
    # Label checks high relative position as WORST/Poor
    if clamped_pos >= 1.0:
        status_str, status_color = "WORST", "darkred"
    elif clamped_pos >= 0.75:
        status_str, status_color = "Poor", "red"
    elif clamped_pos >= 0.50:
        status_str, status_color = "Average", "orange"
    elif clamped_pos >= 0.25:
...
def schedule_blue_tracker_update(*args):
    """Debounces chart updates caused by typing in text boxes to avoid UI lag."""
    global blue_tracker_timer_id
    if blue_tracker_timer_id is not None:
        root.after_cancel(blue_tracker_timer_id)
    # Debounce redraws by 300ms so typing stays fluid
...
# --- GUI SETUP ---
root = tk.Tk()
root.title("ToMT Tier Progress Calculator (v7.3.2)")
