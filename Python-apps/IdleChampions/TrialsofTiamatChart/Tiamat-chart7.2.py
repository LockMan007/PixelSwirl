...
def get_current_plot_filepath():
    ensure_plot_dir()
    # Locks filename to the ISO year and week number (e.g., plot_data_2026_W30.json)
...
def load_all_historical_runs():
    """Loads all JSON plot files in PLOT_DIR and aggregates them into min/max historical bounds."""
    ensure_plot_dir()
    ...
    # Global time grid across full 168 hours
...
def zoom_in():
    ...
    # Proportional 2x scale step for X axis
    span_x = current_x_range[1] - current_x_range[0]
    new_span_x = max(4, span_x / 2.0)
    
    # Target 25% from left edge (top-left focus)
...
def update_chart():
    ...
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda val, loc: f"{val:.1f}%" if y_span <= 10 else f"{int(val)}%"))
    ...
    if history:
        ...
        try:
            ...
            for i in range(len(history)):
                pt_x = history[i][0]
                pt_y = history[i][1]
                pt_dps = float(history[i][2]) if len(history[i]) > 2 else current_live_dps
...
def reset_run():
    ...
    if messagebox.askyesno("Confirm Reset", "Start a new run? This will clear active tracking for a new encounter."):
        history = []
        start_time = None
        total_seconds_at_start = 0
        initial_hp_at_start = 0
        ...
        zoom_reset()
        update_chart()
        messagebox.showinfo("Reset Complete", "Ready for a new run. Enter remaining time/HP and click Calculate!")
...
# Performance Gauge Bar (Floating on Right)
...
tk.Button(button_frame, text="Calculate", command=start_tracking, width=12).pack(side=tk.LEFT, padx=3)
tk.Button(button_frame, text="New Run", command=reset_run, width=12, fg="blue").pack(side=tk.LEFT, padx=3)
tk.Button(button_frame, text="ReLoad Chart", command=reload_chart, width=12, fg="red").pack(side=tk.LEFT, padx=3)
...
# Zoom Controls Row Frame (Above Matplotlib Canvas)
zoom_frame = tk.Frame(root)
zoom_frame.grid(row=5, column=0, columnspan=4, sticky="e", padx=15, pady=(2, 0))

tk.Label(zoom_frame, text="Zoom:", font=("Arial", 8, "bold")).pack(side=tk.LEFT, padx=(0, 3))
tk.Button(zoom_frame, text=" - ", command=zoom_out, width=3, font=("Arial", 8, "bold")).pack(side=tk.LEFT, padx=1)
tk.Button(zoom_frame, text=" + ", command=zoom_in, width=3, font=("Arial", 8, "bold")).pack(side=tk.LEFT, padx=1)
tk.Button(zoom_frame, text=" Reset ", command=zoom_reset, font=("Arial", 8)).pack(side=tk.LEFT, padx=(2, 0))
...
canvas.get_tk_widget().grid(row=6, column=0, columnspan=4, sticky="nsew", padx=10, pady=5)
...
progress_container.grid(row=7, column=0, columnspan=4, sticky="ew", pady=7, padx=10)
