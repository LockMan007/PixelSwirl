...
def load_all_historical_runs():
    ...
    for run in all_runs:
        ...
        # Interpolate run points across the grid
...
def zoom_in():
    ...
    # Target 25% from the left edge instead of middle/last point
    ...
    # Adjust boundaries if hitting the left edge
    if new_min_x == 0:
    ...
    # Target 75% height (top portion of chart)
    ...
    # Adjust boundaries if hitting the top ceiling
    if new_max_y == 100:
...
# --- PLACED HERE SO EVERYTHING ABOVE IT IS DEFINED ---
def load_all():
