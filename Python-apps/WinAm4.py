...
def draw_color_sliders(base_x, base_y, label_prefix="1", colors=None, color_index=0):
    for i, channel in enumerate(["R", "G", "B"]):
        draw_label(f"{label_prefix} {channel}", (base_x, base_y + i * 35))
        ...
        settings_ui[f'slider_{label_prefix}_{channel}'] = {
            ...
            'color_comp': i,
            'min': 0, 'max': 255,
            'color_list_name': 'SPECTRUM_COLORS' if 'spectrum' in label_prefix.lower() else 'OSCILLOSCOPE_COLORS' if 'oscilloscope' in label_prefix.lower() else 'SPECTROGRAM_COLORS'
        }
...
def draw_section(x, y, title, color_preview_color, active_var, gradient_var, colors, color_count_var):
    ...
    for i in range(globals()[color_count_var]):
        ...
        # The WinAm.py logic has one color list, not one for each section
        # So we adapt to use the correct color list for the section
        if title == "Spectrum":
            color_list = SPECTRUM_COLORS
        elif title == "Oscilloscope":
        ...
        if len(color_list) > i:
            draw_color_sliders(x + x_offset, y + 60, f"{i+1}", colors=color_list, color_index=i)
