...
# --- Load Settings ---
def load_settings():
    global GAIN_FACTOR, OSCILLOSCOPE_ACTIVE, SPECTRUM_ACTIVE, BACKGROUND_COLOR, \
...
def draw_settings_screen():
    ...
    for key, ui_element in settings_ui.items():
        ...
        if element_type == 'label':
            ...
            label_surface = settings_font.render(ui_element['label_text'], True, (255, 255, 255))
            text_rect = label_surface.get_rect(center=ui_element['rect'].center)
            is_active = (ui_element['value'] == (SPECTRUM_COLOR_COUNT if 'spectrum' in ui_element['action'] else OSCILLOSCOPE_COLOR_COUNT))
...
def handle_settings_click(pos):
    global app_state, slider_dragging, GAIN_FACTOR, OSCILLOSCOPE_ACTIVE, SPECTRUM_ACTIVE, \
           OSCILLOSCOPE_COLOR_COUNT, SPECTRUM_COLOR_COUNT, OSCILLOSCOPE_GRADIENT_ACTIVE, SPECTRUM_GRADIENT_ACTIVE
...
def handle_slider_drag(pos):
    ...
    if slider_dragging:
        ...
        if var_name == "GAIN_FACTOR":
            ...
            color_list = SPECTRUM_COLORS if 'spectrum' in var_name else OSCILLOSCOPE_COLORS
...
def handle_menu_click(mouse_pos):
    ...
    # Adjust mouse position to be relative to the menu's surface
    relative_mouse_pos = (mouse_pos[0] - menu_rect.x, mouse_pos[1] - menu_rect.y)

    for option in MENU_OPTIONS:
        if 'rect' in option and option['rect'].collidepoint(relative_mouse_pos):
            MENU_ACTIVE = False
            ...
            while running:
                ...
                if app_state == APP_STATE_MAIN:
                    ...
                    if OSCILLOSCOPE_ACTIVE:
                        draw_oscilloscope(screen, audio_data_buffer, OSCILLOSCOPE_COLORS, OSCILLOSCOPE_GRADIENT_ACTIVE)
