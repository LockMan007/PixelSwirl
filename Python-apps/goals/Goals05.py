...
def load_goals():
    ...
    for goal in data['goals']:
        ...
        audio_alert = goal.get('audio_alert', False)
...
class GoalTrackerApp(tk.Tk):
    ...
    def setup_ui(self):
        """Sets up the main application layout using grid."""
        ...
        self.add_button = Button(self, text="Add Goal", command=lambda: self.open_goal_window(None))
    ...
    def load_and_display_goals(self):
        ...
        for i, (goal_id, name, freq_calc, last_date, img_path, audio_alert) in enumerate(goals):
            self.create_goal_column(i, goal_id, name, freq_calc, last_date, img_path, audio_alert)
        
        for i in range(len(goals)):
            self.main_frame.grid_columnconfigure(i, weight=1)
    
    def create_goal_column(self, col_index, goal_id, name, frequency, last_completed_date, image_path, audio_alert):
        """Creates a single column for a goal using the grid manager."""
        column_frame = Frame(self.main_frame, bd=2, relief="groove")
        column_frame.grid(row=0, column=col_index, sticky="nsew", padx=5, pady=5)
...
if __name__ == "__main__":
    ...
    if not os.path.exists('images/default_balloon.png'):
        ...
        # This line has been changed to remove the 10-pixel padding on all sides.
        draw.ellipse((0, 0, 99, 99), fill='skyblue', outline='darkblue', width=2)
