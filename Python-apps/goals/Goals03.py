...
def load_goals():
    ...
    for goal in data['goals']:
        ...
        goals.append((goal_id, name, frequency_days_calc, last_completed_date, image_path))
    return goals

def save_goal(name, days, hours, minutes, image_path):
    """Saves a new goal to the JSON file, storing frequency in parts."""
    with open(JSON_FILE, 'r') as f:
        data = json.load(f)
...
def update_goal(goal_id, last_completed_date):
...
class GoalTrackerApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        add_button = Button(self, text="Add Goal", command=self.open_add_goal_window)
        add_button.pack(pady=5)
        
    def open_add_goal_window(self):
        """Opens a new window for adding a goal with a single form."""
        ...
        def save_and_close():
            try:
                ...
                save_goal(name, days, hours, minutes, image_name)
                add_window.destroy()
                self.load_and_display_goals()
    ...
    def load_and_display_goals(self):
        ...
        goals = load_goals()
        for i, (goal_id, name, freq_calc, last_date, img_path) in enumerate(goals):
            self.create_goal_column(goal_id, name, freq_calc, last_date, img_path, i)
    
    def create_goal_column(self, goal_id, name, frequency, last_completed_date, image_path, column_index):
        """Creates a single column for a goal."""
        column_frame = Frame(self.main_frame, width=150, bd=2, relief="groove")
    ...
    def format_countdown_timer(self, time_left: timedelta) -> str:
        """Formats the timedelta into a concise string."""
    ...
    def reset_goal(self, goal_id):
        ...
        update_goal(goal_id, datetime.now())
        self.load_and_display_goals()
    ...
    def update_timers_and_heights(self):
        ...
        for frame in self.column_frames.values():
            ...
            timer_label.config(text=self.format_countdown_timer(time_left))
